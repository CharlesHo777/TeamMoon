ActiveAdmin.register Message, as: 'Message', namespace: :user do
  ...
  scope :all_messages, default: true do |messages|
    messages.where('recipient_id == ?', current_participant.id)
  end
  scope :from_coordinators do |messages|
    messages.where('recipient_id == ?', current_participant.id).where('sender_id < ?', 0)
  end
  ...
  action_item :reply_to_sender, only: [:show] do
    link_to("Reply To This Sender", new_user_message_path(:message => {:recipient_id => resource.sender_id}))
  end
  ...
  form do |f|
    f.object.sender_id = current_participant.id
    f.inputs do
      ...
      f.input :sender_id, :as => :select, collection: [[current_participant.name, current_participant.id]], :include_blank => false
      f.input :content
    end
    f.actions
  end
  ...
  show do
    panel "Options" do
      link_to("Go Back To Messages", user_messages_path())
    end
